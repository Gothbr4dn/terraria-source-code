...
namespace Terraria.GameContent.UI.States
{
	public abstract class AWorkshopPublishInfoState<TPublishedObjectType> : UIState, IHaveBackButtonCommand
	{
		...
		{
			if (Enumerable.Contains(path, ' ') && path[0] != '\'' && path[0] != '"')
			{
				path = "\"" + path + "\"";
			}
			string[] array = new string[5] { "xdg-open", "gnome-open", "kfmclient", "open", "explorer.exe" };
			foreach (string text in array)
			{
				ProcessStartInfo processStartInfo = new ProcessStartInfo(text, path);
				processStartInfo.ErrorDialog = false;
				try
				{
					Console.WriteLine("Attempting to open " + path + " with " + text);
